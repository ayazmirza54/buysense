...
// Type definitions for the configuration object passed to extendMaterial
...
function extendMaterial(BaseMaterial: new (options?: any) => THREE.Material, cfg: MaterialConfig) {
  ...
  // Copy properties from default material options to uniforms
...
const MergedPlanes = forwardRef<THREE.Mesh, MergedPlanesProps>(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    if (mesh.current && (mesh.current.material as THREE.ShaderMaterial).uniforms) {
      ((mesh.current.material as THREE.ShaderMaterial).uniforms.time as any).value += 0.1 * delta;
    }
  });
